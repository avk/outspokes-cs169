...
describe <%= controller_class_name %>Controller do
  ...
  before do 
    ...
    @login_params = { :<%= login_field %> => 'quentin<%= "@example.com" if options[:email_as_login] %>', :password => 'test' }
    <%= class_name %>.stub!(:authenticate).with(@login_params[:<%= login_field %>], @login_params[:password]).and_return(@<%= file_name %>)
  end
  def do_create
  ...
  describe "on failed login" do
    ...
    it 'flashes an error'           do do_create; flash[:error].should =~ /Couldn't log you in as 'quentin<%= "@example.com" if options[:email_as_login] %>'/ end
